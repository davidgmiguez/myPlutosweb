...
# ╔═╡ 1fcd418c-f36a-4ce8-be92-9955d0eab5b8
md" # 3. Computational Models

Complex systems are described by nonlinear equations, and therefore, very often, they cannot be solved analytically. Therefore, the most common aproach is the use of mathematical models. 
...
# ╔═╡ c932bdb0-bbff-4d4a-9bca-4024d42c1c81
function unconstrained_growth(n, r,final_time)
traj = []
...
# ╔═╡ b694c8c4-ebdf-439a-a7ef-fb0c37a763fa
begin
	scatter(unconstrained_growth(10, 1.05,100))
	title!("Unconstrained growth")
		xlabel!("Time")
		ylabel!("Number of cells")
end

# ╔═╡ 242126c3-691f-4221-9b4c-b6ffa78a4c04
md" This is often an ideal situation where resources are infinite. Most often, the population cannot grow indefinitely, because there exist constraints in terms of food, space or other resources. We can simply update our system to include this restriction, that if the population reaches a certaing size, it stops growing. A very simple way of doing this is to make the growth rate dependent on the population itself (feedback, moving landscape). We start with a straight line:"
...
# ╔═╡ 580ce051-59cb-47bd-bb83-4724db4552c8
begin
...
# ╔═╡ bd691365-e849-4ca1-93b2-9513db9a3b6a
function constrained_growth(n, r,final_time)
traj = []
for t in 1:final_time # arbitrary, just leave enough for it to reach a steady state given the dt
n = n * (1.10 - 0.002 * n)
push!(traj,n)
...
# ╔═╡ 23857eb4-dcdc-4186-8230-a03cef15d622
begin
	scatter(constrained_growth(10, 1.05,100))
	title!("Constrained growth")
		xlabel!("Time")
		ylabel!("Number of cells")
end

# ╔═╡ 717a502f-f7e1-41d6-9ba4-190edf7a88d2
begin
	P[1]=10 # Initial number of cells
	for t = 1:1:final_time-1
		P[t+1] = P[t]  * growth_factor(P[t]);
	end
end

# ╔═╡ edfaf441-ca82-4ee7-9cc2-c27b0f9e0a9a
begin
	P[1]=10 # Initial number of cells
...
# ╔═╡ 0899456f-0d89-4bab-85a1-965089bc5661
begin
	scatter([1:final_time],P[:],line = (:dot, 4))
	title!("Constrained growth")
	xlabel!("Time")
	ylabel!("Number of cells")
end
...
function logistic_growth(n, r; dt=0.01)
traj = []
...
# ╔═╡ 12291cec-662f-4427-b961-05e5b648be4a
AAA=logistic_growth(0.5, r; dt=0.01)
